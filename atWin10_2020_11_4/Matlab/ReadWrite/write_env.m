...
if ( length( Pos.s.z ) >= 2 && equally_spaced( Pos.s.z ) )
    fprintf( fid, '%.2f %.2f', Pos.s.z( 1 ), Pos.s.z( end ) );
else
    fprintf( fid, '%.2f  ', Pos.s.z );
end
...
if ( length( Pos.r.z ) >= 2 && equally_spaced( Pos.r.z ) )
    fprintf( fid, '%.2f %.2f ', Pos.r.z( 1 ), Pos.r.z( end ) );
else
    fprintf( fid, '%.2f  ', Pos.r.z );
end
...
if ( strcmp( model, 'BELLHOP' ) ||  strcmp( model, 'simplePE' ) )
    ...
    if ( length( Pos.r.r ) >= 2 && equally_spaced( Pos.r.r ) )
        fprintf( fid, '%.2f %.2f', Pos.r.r( 1 ), Pos.r.r( end ) );
    else
        fprintf( fid, '%.2f ', Pos.r.r );
    end
    fprintf( fid, '/ \t ! Rr(1)  ... (km) \n' );
    write_bell( fid, Beam );
end
