function bellhopM( filename,loss_db )
...
global Pos
global alpha Nbeams
global U Arr MxNarr Contri_onebeam
...
%        
        ...
        if Beam.RunType(1:1) == 'R'
            ...
            figure(11);clf(11);plot(r,z,'k');
           set( gca, 'xtick',0:5e3:30e3,'xticklabel',0:5:30,'YDir', 'Reverse','Fontsize',16);
            xlabel('Range');ylabel('Depth')
            ylim([0,1100]);
            xlim([0,30e3]);
            
            Contri_onebeam = zeros( length( Pos.r.z ), length( Pos.r.r ) );
            ...
            % Add loss
%              loss_db = -50; 
              extra_loss = 10^(loss_db/20);
                [~,indexRef] = findpeaks(z); % Find Reflection Point
                for ii = 1:length(indexRef)
                    [~,index_range] = min(abs(Pos.r.r - r(indexRef(ii))));
                    Contri_onebeam(:,index_range:end) = Contri_onebeam(:,index_range:end)* extra_loss;
                    %fprintf('声线：%d 反射位置：r:%.1f,z:%.1f \n',ibeam,Pos.r.r(index_range)/1000,z(indexRef(ii)));
                end
            U = U + Contri_onebeam;   
            imagesc
        end
